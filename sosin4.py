...
#고쳐야함 
def long_title(a):
  #메인 함수 내에 제목이 10글자 이상 된다는 조건이 있으면 이 함수를 불러와 준다.
  cnt=0
  ccnt=0
  while(cnt<12):
    lcd_string(mlist[sunse][:8]+ str(" ")*cnt,LCD_LINE_1)
    lcd_string(str(a)+str(" ")*cnt,LCD_LINE_2)
    cnt=cnt+1
  while(ccnt<12):
    lcd_string(mlist[sunse][8:]+ str(" ")*ccnt,LCD_LINE_1)
    lcd_string(str(a)+str(" ")*ccnt,LCD_LINE_2)
    ccnt=ccnt+1
...
def main():
  ...
  os.remove("/home/pi/Downloads/index2.txt")
  indexnum=str(sunse)
